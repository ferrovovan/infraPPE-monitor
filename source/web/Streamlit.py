...
if "stats" not in st.session_state:
    st.session_state.stats = {"total": 0, "helmet_no": 0, "mask_no": 0, "critical": 0}
...
if uploaded_file is not None:
    ...
    with col1:
        ...
        st.image(thermal, caption="ИК-представление (симуляция)", use_column_width=True)
    ...
    with col2:
        ...
        if risk == "low":
            st.markdown(f'<div class="risk-low"><h3>{title}</h3><p>{explanation}</p></div>', unsafe_allow_html=True)
        elif risk == "medium":
            st.markdown(f'<div class="risk-medium"><h3>{title}</h3><p>{explanation}</p></div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="risk-high"><h3>{title}</h3><p>{explanation}</p></div>', unsafe_allow_html=True)
...
st.markdown(
    ...
    "CV-10 • ИИ-контроль СИЗ в ИК-диапазоне • Точность >93% • Работает без интернета"
    "</p>",
    unsafe_allow_html=True
)