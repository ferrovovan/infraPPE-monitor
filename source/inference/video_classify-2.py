...
MODEL_PATH = os.path.join(os.path.dirname(__file__), "../../models/ppe_thermal_efficientnet_b0.pt")

@torch.no_grad()
def load_ppe_model():
    model = models.efficientnet_b0(pretrained=False)
    num_ftrs = model.classifier[1].in_features
    model.classifier[1] = torch.nn.Linear(num_ftrs, 4)  # helmet_yes/no, mask_yes/no
